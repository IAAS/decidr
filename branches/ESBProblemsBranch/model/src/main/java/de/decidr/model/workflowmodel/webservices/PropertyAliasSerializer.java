...
public class PropertyAliasSerializer implements ExtensionDeserializer,
    ...
    private static Logger log = DefaultLogger.getLogger(PropertyAliasSerializer.class);
    ...
    public ExtensibilityElement unmarshall(Class parentType, QName elementType,
        ...
        PropertyAlias extPropertyAlias = (PropertyAlias) extReg.createExtension(parentType,
                elementType);
        PropertyAlias tmpPropertyAlias = null;
        try {
            Unmarshaller u = JAXBContext.newInstance(PropertyAlias.class).createUnmarshaller();
            JAXBElement<PropertyAlias> jaxbElement = u.unmarshal(el, PropertyAlias.class);
            tmpPropertyAlias = jaxbElement.getValue();
        } catch (JAXBException e) {
            log.error("Can't unmarshall PropertyAlias element in "+def.getTargetNamespace(), e);
        }
        if (tmpPropertyAlias != null){
            ...
            if (tmpPropertyAlias.isSetDocumentation()){
                extPropertyAlias.getDocumentation().addAll(tmpPropertyAlias.getDocumentation());
            }
            ...
            if (tmpPropertyAlias.isSetMessageType()){
                extPropertyAlias.setMessageType(extPropertyAlias.getMessageType());
            }
            if (tmpPropertyAlias.isSetPart()){
                extPropertyAlias.setPart(tmpPropertyAlias.getPart());
            }
            if (tmpPropertyAlias.isSetPropertyName()){
                extPropertyAlias.setPropertyName(tmpPropertyAlias.getPropertyName());
            }
    ...
    public void marshall(Class parentType, QName elementType,
        ...
        try {
            ...
            Marshaller m = JAXBContext.newInstance(PropertyAlias.class).createMarshaller();
            JAXBElement<PropertyAlias> jaxbElement = new JAXBElement<PropertyAlias>(
                    new QName(Constants.VARPROP_NAMESPACE,
                            "propertyAlias"), PropertyAlias.class, PropertyAlias);
            m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            m.setProperty(Marshaller.JAXB_FRAGMENT, Boolean.TRUE);
            m.marshal(jaxbElement, pw);
            pw.println();
        } catch (JAXBException e) {
            log.error("Can't marshall the PropertyAlias element in "+def.getTargetNamespace(), e);
        }
