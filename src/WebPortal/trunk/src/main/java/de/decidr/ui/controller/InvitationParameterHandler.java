        ...
        for (Iterator<?> it = parameters.keySet().iterator(); it.hasNext();) {
            ...
            try{
                if(key.equals(DecidrGlobals.URL_PARAM_INVITATION_ID)){
                	...
                	authKey = value;
                }else if(key.equals(DecidrGlobals.URL_PARAM_REGISTRATION_REQUIRED)){
	        	...
	        	if (registrationRequired){
	        		if(userFacade.isRegistered(userId)){
	        			...
	        			if((Long)session.getAttribute("userId") == userId)
	        			{
	        				...
	        				//not logged in
	        				UIDirector.getInstance().getTemplateView().setVerticalNavigation(new LoginComponent(new InvitationDialogComponent(invDescription,invitationId, userId)));
	        				Main.getCurrent().getMainWindow().addWindow(new InformationDialogComponent("Please login to accept this invitation","Login required"));
	        			}
	        		...
	        		if(authKey.equals(""))
	        		{
	        			...
	        			if((Long)session.getAttribute("userId") == userId)
	        			{
	        				...
	        				if (userFacade.isRegistered(userId))
	        				{
	        					...
	        					//not registered
	        					Main.getCurrent().getMainWindow().addWindow(new InformationDialogComponent("Authentication failed: Unknown User.","Authentication failed"));
	        				}
	        			...
	        			if (userFacade.authKeyMatches(userId, authKey))
	        			{
	    	        		...
	    	        		Main.getCurrent().getMainWindow().addWindow(new InformationDialogComponent("Your authentication key does not match!","Authentication failed!"));
	    	        	}
	        		}
	        		}
