...
\hline
\textbf{Kategorie} & \textbf{Berkley-Datenbank} & \textbf{fsfs-Backend} \\ \hline 
Speicherung & Speichert das Repository in einer Datenbank & Speichert das Repository in einer einzigen Datei \\ \hline
Transaktion & Transaktionen werden mittels der Datenbank umgesetzt & Transaktionen werden mittels Unterordner umgesetzt \\ \hline
Zugriff & Die Größe und der Zugriff spielen keine Rolle & Die Größe ist und die Anzahl der Zugriffe sind kleiner als bei der Datenbank \\ \hline
Algorithmus & Benutzt einen $O(n²)$-Algorithmus, um den kompletten Ordner zu überschreiben & Benutzt einen O(n) -Algorithmus, um die Dateien an das Repository ranzuhängen \\ \hline
Checkout & Auscheken der letzten Revision ist schnell & Auschecken der letzten Revision ist langsamer als bei der DB \\ \hline
Fehlerfall & Beim Crash bleibt die DB unbrauchbar bis sie repariert wird & Beim Crash ist nicht das komplette Repository betroffen, sondern evtl. nur die Transaktion \\ \hline
Backup & Repository-Backup funktioniert zur Laufzeit & Repository-Backup funktioniert zur Laufzeit \\ \hline
Portabilität & Repository kann nicht auf andere OS kopiert werden & Repository kann auf andere OS portiert werden\\ \hline
\end{tabular}
\label{tab:dbvsfs}
\caption{Vergleich Berkley-Datenbank-Backend und fsfs-Backend \cite{svn}}
\end{table}
\end{center}


Neben \marginpar{Vorteile} SVN gibt es das Concurrent Version System \cite{cvs} (kurz: CVS) zur Versionskontrolle. Dieses speichert alle Daten an einer zentralen Stelle, dem Repository. Im Gegensatz zu CVS bietet SVN einige Merkmale, die das Arbeiten einfacher gestalten. In der folgenden Liste, sind die Vorteile gegenüber CVS aufgelistet:
\begin{itemize}
